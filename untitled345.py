# -*- coding: utf-8 -*-
"""
Created on Thu Feb 21 15:44:48 2019
...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 20 09:18:29 2019
...
with open('PrasannaToRajSunnidhya.csv') as csf:
    ...
    for row in csv_reader:
        MedianX.append(float(row[3]))
        MedianY.append(float(row[4]))
        MedianZ.append(float(row[5]))
        counter = counter + 1
        sumX = sumX + float(row[3])
        sumY = sumY + float(row[4])
        sumZ = sumZ + float(row[5])
    MedianX.sort()
    MedianY.sort()
    MedianZ.sort()
...
with open('PrasannaToRajSunnidhya.csv') as csf:
    ...
    sumDenominatorZ = 0.0
    for row in csv_reader:
        sumNumeratorX = sumNumeratorX + (float(row[3]) - sumXavg)**4
        sumDenominatorX = sumDenominatorX + (float(row[3]) - sumXavg)**2
        sumNumeratorY = sumNumeratorY + (float(row[4]) - sumYavg)**4
        sumDenominatorY = sumDenominatorY + (float(row[4]) - sumYavg)**2
        sumNumeratorZ = sumNumeratorZ + (float(row[5]) - sumZavg)**4
        sumDenominatorZ = sumDenominatorZ + (float(row[5]) - sumZavg)**2
...
with open('PrasannaToRajSunnidhya.csv') as csf:
    ...
    sum1Z = 0.0
    for row in csv_reader:
        sum1X = sum1X + ((float(row[3]) - sumXavg)**2)
        sum1Y = sum1Y + ((float(row[4]) - sumYavg)**2)
        sum1Z = sum1Z + ((float(row[5]) - sumZavg)**2)
    Vx = sum1X / counter
    Vy = sum1Y / counter
    Vz = sum1Z / counter
...
with open('PrasannaToRajSunnidhya.csv') as csf:
    ...
    Sznumerator = 0.0
    for row in csv_reader:
        Sxnumerator = Sxnumerator + (float(row[3]) - sumXavg)**3
        Sxnumerator = Sxnumerator + (float(row[4]) - sumYavg)**3
        Sxnumerator = Sxnumerator + (float(row[5]) - sumZavg)**3
...
with open('PrasannaToRajSunnidhya.csv') as csf1:
    ...
    for row in csv_reader1:
        sumNumerator = sumNumerator + ((float(row[3]) - sumXavg)*(float(row[4]) - sumYavg)*(float(row[5]) - sumZavg))
        sumXsquare = sumXsquare + (float(row[3]) - sumXavg)*(float(row[3]) - sumXavg)
        sumYsquare = sumYsquare + (float(row[4]) - sumYavg)*(float(row[4]) - sumYavg)
        sumZsquare = sumZsquare + (float(row[5]) - sumZavg)*(float(row[5]) - sumZavg)
